# This app accepts an email address to use to retrieve an Amazon wishlist
...
def find_books(books, library, stream)
  ...
  books.each do |book|
    ...
    html = "<li><span class='bookTitle'>#{book[:title]}</span> is available at "
    if libraries_available.include? LIBRARY_MY_STRING
      book_available = true
      html += "your library.</li>"
      stream << "data: #{html}\n\n"
      #stream << "data: { \"bookTitle\": \"#{book[:title]}\", \"locations\": \"your library\" }\n\n"
    elsif libraries_available.length > 0 && LIBRARY_BRANCH_LOCATION == ''
      book_available = true
      #stream << "data: { \"bookTitle\": \"#{book[:title]}\", \"locations\": \"#{libraries_available.uniq.join(', ')}\" }\n\n"
  ...
  stream << "status: data: No books available.\n\n" unless book_available == true
end
...
get '/retrieve/:email/:library' do
  ...
  stream do |out|
    begin
      ...
      find_books(books, params[:library], out) unless books.nil? || books.empty?
      out << "data: #{nil}\n\n"
    rescue IOError
